...
class Project:
    def __init__(self, project_id: uuid6.UUID):
        self.id = project_id.hex
        self.n_days = 365
        ...
        self.grid_outputs: grid.GridOutput | None = None
        self.supply_outputs: supply.SupplyOutput | None = None
    ...
    def grid_results(self):
        ...
        self.nodes_obj: grid.NodeAttributes = self.grid_outputs.results.nodes
        self.links_obj: grid.Links = self.grid_outputs.results.links
    ...
    def _process_supply_optimization_results(self):
        results = self.supply_outputs.results
        ...
        def get_capacity(component, result_key):
            if not component.settings.is_selected:
                return 0
            return (
                results[result_key].scalars["invest"]
    ...
    def get_results_summary(self) -> ResultsSummary:
        ...
        res.lcoe = self.results.lcoe
        res.capex = (
            self.results.cost_grid
            + self.results.cost_renewable_assets
            + self.results.cost_non_renewable_assets
        )
        res.res = self.results.res
        res.co2_savings = self.results.co2_savings
        res.consumption_total = self.results.total_annual_consumption
...
if __name__ == "__main__":
    ...
    simulation = Project(uuid6.uuid7())
    ...
    grid_output_json = pathlib.Path("./tests/examples/grid_output_example.json").read_text()
    grid_output = grid.GridOutput.model_validate_json(grid_output_json)
    simulation.grid_outputs = grid_output

    supply_output_json = pathlib.Path("./tests/examples/supply_output_example.json").read_text()
    supply_output = supply.SupplyOutput.model_validate_json(supply_output_json)
    ...
    print(simulation.results_summary)
