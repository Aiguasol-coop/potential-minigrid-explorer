...
import pandas as pd
import datetime
from app.explorations.domain import (CategoryDistribution,
                                     PublicServiceData,
                                     HouseholdData,
                                     EnterpriseData,
...
def get_theoretical_distribution(df_centroids: pd.DataFrame, session: sqlmodel.Session) -> dict[str, int]:
    ...
    theoretical = {
        "public_services": round(centroid_category_distribution["public_services"] * df_centroids.shape[0]),
...
def build_demand(subcategory_data_dict: dict[str, dict[str, float]],
                 total_count: int,
                 existing_categories: ExistingPublicBuilding | None = None ) -> dict[str, dict[str, float]]:
...
def adjust_distribution(category_demand: dict[str, dict[str, float]],
    ...
    sorted_categories = sorted(
        ...
        key=lambda kv: kv[1].get("distribution", 0),
        reverse=True
    )
...
def expand_hourly(daily_demand: dict[str, float],
                  session: sqlmodel.Session,
                  ProfileModel : type[HouseholdHourlyProfile] | type[EnterpriseHourlyProfile] | type[PublicServiceHourlyProfile],
                  area_type: str | None = None) -> dict[str, dict[str, float]]:
    ...
    for subcategory in daily_demand.keys():
        ...
        profile_obj : HouseholdHourlyProfile | EnterpriseHourlyProfile | PublicServiceHourlyProfile = session.exec(query).one()
...
def build_annual_demand(household_hourly: dict[str, dict[str, float]],
                        enterprise_hourly: dict[str, dict[str, float]],
                        public_service_hourly: dict[str, dict[str, float]],
                        area_type:str) -> ElectricalDemand:
    ...
    daily_hourly_demand = (
        households_df.add(enterprises_df, fill_value=0).add(public_services_df, fill_value=0).sum(axis=1)  # type: ignore
    )
...
def calculate_demand(cluster_centroids: list[Building], session: sqlmodel.Session) -> ElectricalDemand:
    df_centroids = pd.DataFrame([
        centroid.model_dump(exclude={'geography', 'pg_geography', 'pg_geography_centroid', 'geography_centroid'})
    ...
    existing_categories: ExistingPublicBuilding = ExistingPublicBuilding(
        ...
        num_hospital_first=len(df_centroids[(df_centroids["building_type"] == "hospital") & (df_centroids["category"] == "First")]),
        num_hospital_primary=len(df_centroids[(df_centroids["building_type"] == "hospital") & (df_centroids["category"] == "Primary")]),
        num_hospital_secondary=len(df_centroids[(df_centroids["building_type"] == "hospital") & (df_centroids["category"] == "Secondary")]),
        num_schools=len(df_centroids[df_centroids["building_type"] == "school"]),
        num_school_primary=len(df_centroids[(df_centroids["building_type"] == "school") & (df_centroids["category"] == "primary")]),
        num_school_secondary=len(df_centroids[(df_centroids["building_type"] == "school") & (df_centroids["category"] == "secondary")]),
    )
    ...
    households_subcategory_data = list(session.exec(
        sqlmodel.select(HouseholdData).where(HouseholdData.area_type == area_type)
    ).all())
    households_subcategory_data_dict = {sub.subcategory: sub.as_dict for sub in households_subcategory_data}

    enterprise_subcategory_data = list(session.exec(
        sqlmodel.select(EnterpriseData).where(EnterpriseData.area_type == area_type)
    ).all())
    enterprise_subcategory_data_dict = {sub.subcategory: sub.as_dict for sub in enterprise_subcategory_data}

    public_services_subcategory_data = list(session.exec(
        sqlmodel.select(PublicServiceData).where(PublicServiceData.area_type == area_type)
    ).all())
    public_services_subcategory_data_dict = {sub.subcategory: sub.as_dict for sub in public_services_subcategory_data}

    household_demand = build_demand(households_subcategory_data_dict, theoretical_distribution["households"])
    enterprise_demand = build_demand(enterprise_subcategory_data_dict, theoretical_distribution["enterprises"])
    public_service_demand = build_demand(public_services_subcategory_data_dict, theoretical_distribution["public_services"], existing_categories)
    ...
    household_demand = adjust_distribution(household_demand, households_subcategory_data_dict,
                                           theoretical_distribution["households"], total_households)
    enterprise_demand = adjust_distribution(enterprise_demand, enterprise_subcategory_data_dict,
                                            theoretical_distribution["enterprises"], total_enterprises)
    public_service_demand = adjust_distribution(public_service_demand, public_services_subcategory_data_dict,
                                                theoretical_distribution["public_services"], total_public)


    total_household_daily = {s: d["kwh_per_day"] * d["consumers"] for s, d in household_demand.items()}
    total_enterprise_daily = {s: d["kwh_per_day"] * d["consumers"] for s, d in enterprise_demand.items()}
    total_public_daily = {s: d["kwh_per_day"] * d["consumers"] for s, d in public_service_demand.items()}


    total_household_hourly = expand_hourly(total_household_daily, session, HouseholdHourlyProfile, area_type)
    total_enterprise_hourly = expand_hourly(total_enterprise_daily, session, EnterpriseHourlyProfile)
    total_public_service_hourly = expand_hourly(total_public_daily, session, PublicServiceHourlyProfile)


    result = build_annual_demand(total_household_hourly, total_enterprise_hourly, total_public_service_hourly, area_type)
...
if __name__ == "__main__":
    ...
    mozambique_bbox = (38.803945,-13.966835,38.807348,-13.964998) #(38.777312,-13.983961,38.859709,-13.943647)  # (min_lon, min_lat, max_lon, max_lat)
    min_lon, min_lat, max_lon, max_lat = mozambique_bbox

    envelope = geofunc.ST_MakeEnvelope(min_lon, min_lat, max_lon, max_lat, 4326)
    centroid_geom : ColumnElement[Any] = sqlalchemy.cast(Building.pg_geography_centroid, Geometry(geometry_type="POINT", srid=4326))

    with sqlmodel.Session(get_engine()) as session:
        cluster_centroids : list[Building] = list(session.exec(sqlmodel.select(Building).where(geofunc.ST_Within(centroid_geom, envelope))).all())
